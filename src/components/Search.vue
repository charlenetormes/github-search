...
export default {
    ...
    methods: {
        async setSearch() {
            ...
            if (this.store.state.type === 'Users') await this.store.dispatch('setResults', { user: this.store.state.search });
            else await this.store.dispatch('setResults', { repo: this.store.state.search });
